...
CS1 = plt.contour(YC[plta:pltb,int(icx/2)]*1e-3,RC[:pltdepth].squeeze(),W_ta[:pltdepth,plta:pltb,icx]*conv_factor, levels, colors='0.6')
...
plt.savefig('./figures/W_timeav_mday_day%d-%d.png' % (day_s, day_e))
#plt.close()
...
plt.ylabel("Depth [m]")
plt.title('Inside eddy, at y~Rmax')
plt.grid(True)
...
plt.ylim(-300,0)
#plt.ylabel("Depth [m]")
plt.title('Inside eddy, %d km from core' % (abs(XC[110,110]-XC[90,90])*1e-3))
ax1.plot(W_ta[:,90,90]*conv_factor, RC[:].squeeze())#, label='timestep %d hr' % (it*timestep/3600))
ax2.plot(W_ta[:,110,110]*conv_factor, RC[:].squeeze())#, label='timestep %d hr' % (it*timestep/3600))
plt.tight_layout(pad=1)
#ax2.legend(loc='lower right')
plt.savefig('./figures/Wmean_profile.png')
