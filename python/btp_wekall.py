...
for it in itrs:
    ...
    Ut_all[int((it-itrs[0])), :,:,:] = U
    Vt_all[int((it-itrs[0])), :,:,:] = V 
    W5d[int((it-itrs[0])), :,:,:]= W;
# ========= trial
...
#       define eddy center
    ...
    XGi = XG-c[0]*1e3+XG[165,165]
    YGi = YG-c[1]*1e3+YG[165,165]
    ...
    for z in np.arange(0,nr):
        ...
        for x in np.arange(0,nx):
            Vt_c[z,:,x] = np.interp(YC[:,x],YGi[:,x],V[z,:,x]) #verify if V points = YUg
...
# Interpolate U_theta
    thetaC = np.linspace(0,2.0*np.pi, 100, endpoint=False)
    radC = np.linspace(0,400,400)
    U_theta = 
# ============== trial end
#
    """
...
#    if (((it+1) % itpd)==0):# and (it-itrs[0] != 0):
    if ((it+1)*ts) in iters1:# and (it-itrs[0] != 0):
        Vort_da[int((it+1-itrs[0])/(tst/ts))-1,:,:]=np.nanmean(Vort_all[(it-itrs[0]-3):(it+1-itrs[0]),:,:], axis=0)
        U_da[int((it+1-itrs[0])/(tst/ts))-1,:,:,:]=np.nanmean(Ut_all[(it-itrs[0]-3):(it+1-itrs[0]),:,:], axis=0)
        V_da[int((it+1-itrs[0])/(tst/ts))-1,:,:,:]=np.nanmean(Vt_all[(it-itrs[0]-3):(it+1-itrs[0]),:,:], axis=0)
