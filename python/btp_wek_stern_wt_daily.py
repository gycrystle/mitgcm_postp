...
#        My_WT = -np.max(taux)/(rho0)*(f0+Ro*Omega+Ro*2*Omega*np.sin(thetaC)**2+Ro*Vort_cnd*np.cos(thetaC)**2)/((f0+Ro*2*Omega)*(f0+Ro*Vort_cnd)-Ro**2*Omega**2)
        Mx_WT = np.max(taux)/(rho0*f0)*Ro*(Vort_cnd-2*Omega)*np.sin(thetaC)*np.cos(thetaC)/((1+Ro*2*Omega)*(1+Ro*Vort_cnd)-Ro**2*Omega**2)
...
dxMx_WT = np.diff(Mx_WTall, axis=2)/np.tile(dXC[:,1:],[6,1,1]) # V-point
