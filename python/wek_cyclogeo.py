...
Mx_g300 = np.trapz(Ue_g300[:,0:idepth,:,:],-RC[0:idepth].squeeze(), axis=1)
My_g300 = np.trapz(Ve_g300[:,0:idepth,:,:],-RC[0:idepth].squeeze(), axis=1)
...
dxMx_g300 = np.diff(Mx_g300, axis=2)/np.tile(dXC[:,1:],[nit,1,1])
dyMy_g300 = np.diff(My_g300, axis=1)/np.tile(dYC[1:,:],[nit,1,1])
...
dxMx_g300i=np.zeros((nit,ny,nx))
dyMy_g300i=np.zeros((nit,ny,nx))
...
for it in np.arange(0,nit):
    for y in np.arange(0,ny):
        ...
        dxMx_g300i[it,y,:] = np.interp(XC[y,:],XUg[y,:-1],dxMx_g300[it,y,:])
        dxMx_cygi[it,y,:] = np.interp(XC[y,:],XUg[y,:-1],dxMx_cyg[it,y,:])
    for x in np.arange(0,nx):
        dyMy_gi[it,:,x] = np.interp(YC[:,x],YUg[:-1,x],dyMy_g[it,:,x])
        dyMy_g300i[it,:,x] = np.interp(YC[:,x],YUg[:-1,x],dyMy_g300[it,:,x])
...
We_g300 = np.mean((dxMx_g300i + dyMy_g300i), axis=0)
