...
dir0 = '/home/cgreace/MITgcm/MITgcm/mitgcm_configs/eddy_airsea/run41_bt_a25_ua/'
dir0 = './'
...
vdissh = mit.rdmds(dir0 + file2,iters2[i],rec=0)
vdissv = mit.rdmds(dir0 + file2,iters2[i],rec=1)
vadv   = mit.rdmds(dir0 + file2,iters2[i],rec=2)
vcori  = mit.rdmds(dir0 + file2,iters2[i],rec=3)
vext   = mit.rdmds(dir0 + file2,iters2[i],rec=4)
vpress = mit.rdmds(dir0 + file2,iters2[i],rec=5)
v_ab   = mit.rdmds(dir0 + file2,iters2[i],rec=6)
vtot   = mit.rdmds(dir0 + file2,iters2[i],rec=7)

vvel0  = mit.rdmds(dir0 + file2,iters2[np.min([i-1,0])],rec=8)
vvel1  = mit.rdmds(dir0 + file2,iters2[i]  ,rec=8)
...
ucori2 = 0.*ucori
ucori2[:,:-1,:] = 0.25*(vvel1[:,1:,:] + vvel1[:,:-1,:])
ucori2[:,:-1,1:] += 0.25*(vvel1[:,1:,:-1] + vvel1[:,:-1,:-1])
