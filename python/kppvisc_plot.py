...
ipdepth = 65
idepth = 14 # depth for subsurface drifts 36 for approx -99 in 75 grid 

#itrs = np.arange(0, 21610, file_dit)
itrs = [60,3600,7200,10800, 14400, 18000, 21600] #, 2160, 2880, 3600, 4320, 5040, 5760, 6480, 7200]
...
ke_range = np.linspace(0,0.15,100, endpoint=True)
ke_ticks = np.linspace(0,0.15,11, endpoint=True)
#ke_range = 99
#ke_ticks = 11
...
#    ax2.semilogx(viscKPP[:65,90,90], RC[:65].squeeze(), label='timestep %d hr' % (it*timestep/3600))
    ax1.plot(viscKPP[:65,165,165], RC[:65].squeeze())#, label='timestep %d hr' % (it*timestep/3600))
    ax2.plot(viscKPP[:65,90,90], RC[:65].squeeze(), label='timestep %d hr' % (it*timestep/3600))
