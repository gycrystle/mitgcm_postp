...
#
ax1 = plt.subplot(231)
plt.contourf(xc_dom,yc_dom,W_ta[17,plta:pltb,plta:pltb]*1e3,we_range,cmap=cm.seismic)#
...
ax1.set_aspect(1)
plt.text(posext,posext,'$W_{extrema}=$ [%1.3f, %1.3f] $mm/s$' % (np.min(W_ta[17,:,:])*1e3,np.max(W_ta[17,:,:])*1e3), fontsize=10)
...
plt.title(r'Time averaged, $\overline{W}$ at $z=%d$, day %d-%d' % (RC[17], dstart, dend),fontsize=11)
# ===============================================
ax2 = plt.subplot(232, sharex=ax1)
plt.contourf(xc_dom,yc_dom,W_ekman[plta:pltb,plta:pltb]*1e3,we_range,cmap=cm.seismic)#
cb=plt.colorbar(ticks=we_ticks, format='%1.2f')
cb.set_label(r'$W \, (mm/s)$', labelpad=-40, y=1.1, rotation=0)
rmax1 = plt.Circle((XC[icy,icx]*1e-3, YC[icy,icx]*1e-3), Rmax*1e-3, color='r',ls=':', fill=False)
...
#plt.clabel(CS2, fmt='%2.2f', colors='k', fontsize=10)
ax2.set_aspect(1)
plt.text(posext,posext,'$W_{extrema}=$ [%1.3f, %1.3f] $mm/s$' % (np.min(W_ekman)*1e3,np.max(W_ekman)*1e3), fontsize=10)
#plt.text(posext,posext,'$W_{extrema}=$ [%1.3f, %1.3f] $mm/s$' % (np.min(W_Bm)*1e3,np.max(W_Bm)*1e3), fontsize=10)
#plt.xlabel("x (km)")
plt.ylabel("y (km)")
#plt.title('Vertically averaged W at $z[0, %d]m$, timestep %d hr' % (RC[idepth],int(it*timestep/3600)),fontsize=11)
...
ax3.set_aspect(1)
im=plt.contourf(xc_dom,yc_dom,Wm_stern[plta:pltb,plta:pltb]*1e3,we_range,cmap=cm.seismic)#
...
plt.text(posext,posext,'$W_{extrema}=$ [%1.3f, %1.3f] $mm/s$' % (np.min(Wm_stern)*1e3,np.max(Wm_stern)*1e3), fontsize=10)
...
plt.contourf(xc_dom,yc_dom,W_WT[plta:pltb,plta:pltb]*1e3,we_range,cmap=cm.seismic)#
...
#plt.clabel(CS2, fmt='%2.2f', colors='k', fontsize=10)
ax4.set_aspect(1)
plt.text(posext,posext,'$W_{extrema}=$ [%1.3f, %1.3f] $mm/s$' % (np.nanmin(W_WT)*1e3,np.nanmax(W_WT)*1e3), fontsize=10)
#plt.text(posext,posext,'$W_{extrema}=$ [%1.3f, %1.3f] $mm/s$' % (np.min(W_Bm)*1e3,np.max(W_Bm)*1e3), fontsize=10)
plt.xlabel("x (km)")
plt.ylabel("y (km)")
#    plt.title('Vertically averaged W at $z[0, %d]m$, timestep %d hr' % (RC[idepth],int(it*timestep/3600)),fontsize=11)
plt.title(r'$\overline{W_{e}}=\overline{\partial_x \int \, U_e + \partial_y \int \, V_e}$ day %d-%d' % (dstart, dend),fontsize=11)
plt.title("Ekman pumping, Wenegrat & Thomas",fontsize=11)
# ===============================================
#
ax5 = plt.subplot(223)
#ax5.set_aspect(1)
plt.plot(yc_dom[:,icx],W_ta[17,plta:pltb,icx]*1e3, label='Time averaged')
plt.plot(yc_dom[:,icx],W_ekman[plta:pltb,icx]*1e3, label=r'$\overline{\partial_x \int \, U_e + \partial_y \int \, V_e}$')
plt.plot(yc_dom[:,icx],Wm_stern[plta:pltb,icx]*1e3, label='Stern')
plt.plot(yc_dom[:,icx],W_WT[plta:pltb,icx]*1e3, label='Wenegrat&Thomas')
plt.legend()
plt.xlabel("y (km)")
plt.ylabel("Vertical velocity (mm/s)")
#
#plt.tight_layout(pad=1)
plt.savefig('./figures/W_ek_btp_day%d-%d.png' % (dstart, dend))
