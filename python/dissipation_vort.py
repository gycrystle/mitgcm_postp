...
#run8 = './run81b_bc_a25_kpp_ptracer/'
...
svortc1 = np.zeros((nitbt))
svortc2 = np.zeros((nitbt))
svortc3 = np.zeros((nitbt))
svortc4 = np.zeros((nitbc))
svortc5 = np.zeros((nitbc))
svortc6 = np.zeros((nitbc))
svortc7 = np.zeros((nitbc))
svortc8 = np.zeros((nitbc))
svortc9 = np.zeros((nitbc))
...
for it in itrsbt:
    ...
    zeta0 = vorticity(U0[0,:,:], V0[0,:,:], dXCbt, dYCbt)
    svortc0[int((it-itrsbt[0])/tsbt)]=np.max(abs(zeta0))

    U1 = mit.rdmds(run1+'U',it)
    V1 = mit.rdmds(run1+'V',it)
    zeta1 = vorticity(U1[0,:,:], V1[0,:,:], dXCbt, dYCbt)
    svortc1[int((it-itrsbt[0])/tsbt)]=np.max(abs(zeta1))

    U2 = mit.rdmds(run2+'U',it)
    V2 = mit.rdmds(run2+'V',it)
    zeta2 = vorticity(U2[0,:,:], V2[0,:,:], dXCbt, dYCbt)
    svortc2[int((it-itrsbt[0])/tsbt)]=np.max(abs(zeta2))

    U3 = mit.rdmds(run3+'U',it)
    V3 = mit.rdmds(run3+'V',it)
    zeta3 = vorticity(U3[0,:,:], V3[0,:,:], dXCbt, dYCbt)
    svortc3[int((it-itrsbt[0])/tsbt)]=np.max(abs(zeta3))
...
for it in itrsbc:
    ...
    zeta4 = vorticity(U4[0,:,:], V4[0,:,:], dXCbc, dYCbc)
    svortc4[int((it-itrsbc[0])/tsbc)]=np.max(abs(zeta4))
#
    U5 = mit.rdmds(run5+'U',it)
    V5 = mit.rdmds(run5+'V',it)
    zeta5 = vorticity(U5[0,:,:], V5[0,:,:], dXCbc, dYCbc)
    svortc5[int((it-itrsbc[0])/tsbc)]=np.max(abs(zeta5))
#
    U6 = mit.rdmds(run6+'U',it)
    V6 = mit.rdmds(run6+'V',it)
    zeta6 = vorticity(U6[0,:,:], V6[0,:,:], dXCbc, dYCbc)
    svortc6[int((it-itrsbc[0])/tsbc)]=np.max(abs(zeta6))
#
    U7 = mit.rdmds(run7+'U',it)
    V7 = mit.rdmds(run7+'V',it)
    zeta7 = vorticity(U7[0,:,:], V7[0,:,:], dXCbc, dYCbc)
    svortc7[int((it-itrsbc[0])/tsbc)]=np.max(abs(zeta7))
#
    U8 = mit.rdmds(run8+'U',it)
    V8 = mit.rdmds(run8+'V',it)
    zeta8 = vorticity(U8[0,:,:], V8[0,:,:], dXCbc, dYCbc)
    svortc8[int((it-itrsbc[0])/tsbc)]=np.max(abs(zeta8))
#
    U9 = mit.rdmds(run9+'U',it)
    V9 = mit.rdmds(run9+'V',it)
    zeta9 = vorticity(U9[0,:,:], V9[0,:,:], dXCbc, dYCbc)
    svortc9[int((it-itrsbc[0])/tsbc)]=np.max(abs(zeta9))
...
plt.plot(timebt,svortc0/f0, color='k',label=run0_label)
plt.plot(timebt,svortc1/f0, color='k',ls='--', label=run1_label)
plt.plot(timebt,svortc2/f0, color='b',label=run2_label)
plt.plot(timebt,svortc3/f0, color='b',ls='--', label=run3_label)
plt.plot(timebc,svortc4/f0, color='0.6', label=run4_label)
plt.plot(timebc,svortc5/f0, color='0.6',ls='--', label=run5_label)
plt.plot(timebc,svortc6/f0, color='c', label=run6_label)
plt.plot(timebc,svortc7/f0, color='c',ls='--', label=run7_label)
plt.plot(timebc,svortc8/f0, color='r', label=run8_label)
plt.plot(timebc,svortc9/f0, color='r',ls='--', label=run9_label)
...
plt.savefig("./figures/dissipation_vorticity_all_run51b.png")
