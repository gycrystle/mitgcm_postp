...
Vort_da = np.zeros((int(nit/8),ny,nx));
W5d = 0.0*Vort_all
...
for it in itrs:
    ...
    Vort_all[int((it-itrs[0])), :,:] = Vort
    W5d[int((it-itrs[0])), :,:,:]= W;
#
    """
# === To compute U ageostrophic from U geostrophic ===

    P = mit.rdmds('PH',((it+1)*ts))
    dxP = np.diff(P, axis=2)/dX_Ug #dX_Ug = np.tile(dXC[:,1:],[nr,1,1])
    Vg = dxP/f0

    dyP = np.diff(P, axis=1)/dY_Vg #dY_Vg=np.tile(dYC[1:,:],[nr,1,1])
    Ug = -dyP/f0
...
# === To compute U ageostrophic from U bottom
    ...
    if (((it+1) % itpd)==0):# and (it-itrs[0] != 0):
        Vort_da[int((it+1-itrs[0])/itpd)-1,:,:]=np.nanmean(Vort_all[(it-itrs[0]-7):(it+1-itrs[0]),:,:], axis=0)
...
W_sternall = np.zeros((nitd,ny,nx));
W_b = 0.0*W_sternall
Mx_WTall = np.zeros((nitd,ny,nx));
My_WTall = np.zeros((nitd,ny,nx));
Vort_cndall = np.zeros((nitd,ny,nx));
Omega_all = np.zeros((nitd,ny,nx));
...
# ----------------------- #1 Stern -----------------------------------
#    if (it % itpd)==0:
    taux = mit.rdmds('diagTAUX',(it*tsd))
    tauy = mit.rdmds('diagTAUY',(it*tsd))
#    Vort_da=np.nanmean(Vort_all[(it-itrsd[0]-8):(it-itrs[0]),:,:], axis=0)
#
    for i in range(1,nx-1):
        for j in range(1,ny-1):
            zeta_intx[j,i] =(Vort_da[it-itrsd[0],j,i]+Vort_da[it-itrsd[0],j+1,i])/2 #vorticity @ u point
            zeta_inty[j,i] =(Vort_da[it-itrsd[0],j,i]+Vort_da[it-itrsd[0],j,i+1])/2
#
...
#    W_sternall[int((it-itrs[0])/itpd),:,:]= W_stern; #u-point u_wt
    W_sternall[int(it-itrsd[0]),:,:]= W_stern; #u-point u_wt
# ------------------------------------------------------------------- 
# --------------------  Wenegrat & Thomas ---------------------------
    Vort300 = vorticity(U[99,:,:],V[99,:,:],dXC,dYC)
    Vort_use = Vort300 # or Vort_da[it-itrsd[0],:,:]
...
#
    Mx_WTall[int((it-itrsd[0])),:,:]=Mx_WT # @cell center
    My_WTall[int((it-itrsd[0])),:,:]=My_WT # @cell center
# -------------------------------------------------------------------
...
# Wenegrat & Thomas
dxMx_WT = np.diff(Mx_WTall, axis=2)/np.tile(dXC[:,1:],[nitd,1,1]) # V-point
dyMy_WT = np.diff(My_WTall, axis=1)/np.tile(dYC[1:,:],[nitd,1,1]) # V-point
# Model produced
...
dxMx_WTi=np.zeros((nitd,ny,nx))
dyMy_WTi=np.zeros((nitd,ny,nx))
for it in np.arange(0,nitd):
    for y in np.arange(0,ny):
...
plt.ylabel("y (km)")
#    plt.title('Vertically averaged W at $z[0, %d]m$, timestep %d hr' % (RC[idepth],int(it*timestep/3600)),fontsize=11)
plt.title(r'$\overline{W_{b}}$, Bruno calculation',fontsize=10)
# ===============================================
