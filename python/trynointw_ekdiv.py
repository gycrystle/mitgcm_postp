...
XUg = YC+0.5*dYC # grid where Ug calculated
...
#Interpolate Ug in cell center, losing first grids(bottom and left)
    ...
    for z in np.arange(0,nr):
        for y in np.arange(0,ny):
            ...
            Ut_c[z,y,:] = np.interp(XC[y,:],XUg[y,:],U[z,y,:])
        for x in np.arange(0,nx):
            Ug_c[z,:,x] = np.interp(YC[:,x],YUg[:-1,x],Ug[z,:,x])
            Vt_c[z,:,x] = np.interp(YC[:,x],YUg[:,x],V[z,:,x]) #verify if V points = YUg
    """
    Vg_all[(it-itrs[0]),:,:,1:]= Vg #cell centered Vg
    Ug_all[(it-itrs[0]),:,1:,:]= Ug #cell centered Ug
    Ut_all[(it-itrs[0]),:,:,:]= U; #cell centered u_total
    Vt_all[(it-itrs[0]),:,:,:]= V; #cell centered v total
#======= time loop end
...
KEg = np.trapz((Ug_all[:,0:idepth,:,:]**2 + Vg_all[:,0:idepth,:,:]**2),-RC[0:idepth].squeeze(),axis=1)
KEag = np.trapz((Ue[:,0:idepth,:,:]**2 + Ve[:,0:idepth,:,:]**2),-RC[0:idepth].squeeze(),axis=1)
...
plt.contourf(XC[1:,1:]*1e-3,YC[1:,1:]*1e-3,W_Bm,w_range,cmap=cm.seismic)
...
plt.plot(time,KEg[:,int(si_y/2+21),int(si_y/2+21)])
plt.plot(time,KEag[:,int(si_y/2+21),int(si_y/2+21)])
plt.ylabel(r'$KE$')
