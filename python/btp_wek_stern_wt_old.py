...
for it in itrs:
    ...
    #transform to polar coord
    eddy_center = np.where(abs(Vort300) == np.max(abs(Vort300))) 
    thetaC = np.arctan2(YC-YC[eddy_center[0],eddy_center[1]], XC-XC[eddy_center[0],eddy_center[1]])
    radC = np.hypot(XC-XC[eddy_center[0],eddy_center[1]],YC-YC[eddy_center[0],eddy_center[1]])
...
dxMx_WT = np.diff(Mx_WTall, axis=2)/np.tile(dXC[:,1:],[6,1,1]) # U-point
...
#    plt.title('Vertically averaged W at $z[0, %d]m$, timestep %d hr' % (RC[idepth],int(it*timestep/3600)),fontsize=11)
plt.title(r'$\overline{W_{e}}=\overline{\partial_x \int \, U_e + \partial_y \int \, V_e}$ day %d-%d' % (dstart, dend),fontsize=11)
plt.title("Ekman pumping computed with Wenegrat & Thomas's formula",fontsize=11)
# ===============================================
#
ax4 = plt.subplot(224, sharex=ax1)
ax4.set_aspect(1)
plt.contourf(xc_dom,yc_dom,(W_ekman[plta-1:pltb-1, plta-1:pltb-1]-Wm_stern[plta:pltb,plta:pltb])*1e3,we_range,cmap=cm.seismic)#
cb=plt.colorbar(ticks=we_ticks, format='%1.2f')
cb.set_label(r'$W \, (mm/s)$', labelpad=-40, y=1.1, rotation=0)
rmax2 = plt.Circle((XC[icy,icx]*1e-3, YC[icy,icx]*1e-3), Rmax*1e-3, color='r',ls=':', fill=False)
ax4.add_artist(rmax2)
rmax22 = plt.Circle((XC[icy,icx]*1e-3, YC[icy,icx]*1e-3), Rmax*2e-3, color='r',ls=':', fill=False)
ax4.add_artist(rmax22)
#CS1 = plt.contour(YC[plta:pltb,int(si_x/2)]*1e-3,RC.squeeze(),W[:,plta:pltb,int(si_x/2)]*1e3, levels, colors='0.6')
#plt.clabel(CS1, fmt='%2.2f', colors='k', fontsize=10)
plt.text(posext,posext,'$W_{extrema}=$ [%1.3f, %1.3f] $mm/s$' % (np.min(W_ekman-Wm_stern[1:,1:])*1e3,np.max(W_ekman-Wm_stern[1:,1:])*1e3), fontsize=10)
#    plt.title('Vertical velocity $W_{num}$ at $x=%dkm$, timestep %d hr' % (XC[int(si_x/2),int(si_x/2)]*1e-3,int(it*timestep/3600)), fontsize=11)
#plt.title(r'$\overline{W_{stern}}=\overline{\frac{1}{\rho_0} \nabla \times \left[\frac{\tau}{f+\zeta}\right]}$', fontsize=14)
plt.title('We_num - We_Stern',fontsize=11)
#plt.text(520,-320,'$W_{extrema}=$[%1.3f, %1.3f] $mm/s$' % (wmin, wmax), fontsize=10)
